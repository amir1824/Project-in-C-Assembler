 ...
 * Processes a single line in the second pass
  ...
  ic A pointer to instruction counter
 code_img Code image
 ...
 * Find the symbol that need replacment in a code line, and replacing them by the address in the symbol table.
  line The current code line that is being processed
  ic A pointer to the current instruction counter
 code_img The machine code image array
  data_table The data symbol table
  code_table The code symbol table
  ext_table The externals symbol table
  ext_references A pointer to the external symbols references table
  whether succeeded
 */
...
/*
  ...
  curr_ic Current instruction pointer of source code line
 ic Current instruction pointer of source code line start
